...
namespace UnityEditor.XR.Interaction.Toolkit.Samples.ARStarterAssets.Editor
{
    ...
    static class ARStarterAssetsSampleProjectValidation
    {
        ...
        static readonly PackageVersion s_ARFPackageMinVersion = new PackageVersion(
            k_ARFPackageMinVersionString
        );
        ...
        static readonly List<BuildValidationRule> s_BuildValidationRules =
            new List<BuildValidationRule>
            {
                ...
                {
                    IsRuleEnabled = () =>
                        s_ARFPackageAddRequest == null || s_ARFPackageAddRequest.IsCompleted,
                    Message =
                        $"[{k_SampleDisplayName}] AR Foundation ({k_ARFPackageName}) package must be installed or updated to use this sample.",
                    Category = k_Category,
                    CheckPredicate = () =>
                        PackageVersionUtility.GetPackageVersion(k_ARFPackageName)
                        >= s_ARFPackageMinVersion,
                    FixIt = () =>
                    {
                        ...
                        {
                            var version = searchResult
                                .Result.Where(
                                    (info) => string.Compare(k_ARFPackageName, info.name) == 0
                                )
                            ...
                            if (!string.IsNullOrEmpty(version))
                            {
                                ...
                                {
                                    Debug.LogError(
                                        $"Package installation error: {k_ARFPackageMinVersionString}@{version} is below the minimum version of {k_ARFPackageMinVersionString}. Please install manually from Package Manager or update to a newer version of the Unity Editor."
                                    );
                                    return;
                                }
                            }
                        ...
                        {
                            Debug.LogWarning(
                                $"Timeout trying to get package list after {k_TimeOutInSeconds} seconds."
                            );
                        }
                        ...
                        {
                            Debug.LogError(
                                $"Package installation error: {s_ARFPackageAddRequest.Error}: {s_ARFPackageAddRequest.Error.message}"
                            );
                        }
                    },
                    FixItAutomatic = true,
                ...
                new BuildValidationRule
                {
                    ...
                    Category = k_Category,
                    CheckPredicate = () =>
                        ProjectValidationUtility.SampleImportMeetsMinimumVersion(
                            k_Category,
                            k_StarterAssetsSampleName,
                            ProjectValidationUtility.minimumXRIStarterAssetsSampleVersion
                        ),
                    FixIt = () =>
                    {
                        if (
                    ...
                    FixItAutomatic = true,
                    Error = !ProjectValidationUtility.HasSampleImported(
                        k_Category,
                        k_StarterAssetsSampleName
                    ),
                },
        ...
        static string GetImportSampleVersionMessage(
        ...
        )
        {
            if (
                ...
                || !ProjectValidationUtility.HasSampleImported(packageFolderName, sampleDisplayName)
            )
                return string.Empty;
