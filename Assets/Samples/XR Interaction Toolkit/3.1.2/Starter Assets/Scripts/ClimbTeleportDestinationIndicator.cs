...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class ClimbTeleportDestinationIndicator : MonoBehaviour
    {
        ...
        protected void OnEnable()
        {
            ...
            {
                if (
                    !ComponentLocatorUtility<ClimbTeleportInteractor>.TryFindComponent(
                        out m_ClimbTeleportInteractor
                    )
                )
                {
        ...
        protected void OnDisable()
        {
            ...
            {
                m_ActiveTeleportVolume.destinationAnchorChanged -=
                    OnClimbTeleportDestinationAnchorChanged;
        ...
        void OnClimbTeleportDestinationAnchorChanged(TeleportationMultiAnchorVolume teleportVolume)
        {
            ...
            var cameraTrans = teleportVolume
            ...
            var destinationDirectionInScreenSpace = cameraTrans.InverseTransformDirection(
                destinationPosition - cameraPosition
            );
            destinationDirectionInScreenSpace.z = 0f;
            var pointerDirection = cameraTrans
                .TransformDirection(destinationDirectionInScreenSpace)
                .normalized;
            m_PointerInstance.position = destinationPosition - pointerDirection * m_PointerDistance;
            m_PointerInstance.rotation = Quaternion.LookRotation(
                pointerDirection,
                -cameraTrans.forward
            );
        }
